  ...
  return(
    <section className={showOneDay? "oneDay-container active":"oneDay-container"}>
      <span className="oneDay-entete">
        <h3><DayOfWeek today={myArray.daily.sunset[btn]}/></h3>
        <button className="btn-close" onClick={handleClick}>Retour</button>
      </span>
          ...
          {myArray && intervals && intervals.map((el)=>{
            ...
            return(
              <span key={el} className="oneDay-card">
                ...
                <div className="tendance">{myArray.daily && <h4>{omm_codes.filter(el => el.code === myArray?.daily?.weather_code[btn])[0].abrege}</h4>}</div>
