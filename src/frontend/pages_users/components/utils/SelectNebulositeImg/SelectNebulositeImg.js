...
function SelectNebulositeImg(props){
  const {cloud_cover_low, precipitation, setNebulositeText, setNebulositeImg} = props;

  console.log("cloud_cover_low ",cloud_cover_low," precipitation  ", precipitation);
  useEffect(() =>{
  const nebulositeArray = [
    ...
    {name: tres_nuageux, text:"ciel couvert"},
    {name: couvert, text:"ciel bouché"},
    {name: orageux, text:"Nuages lourds"}
  ];
  const seuilsArray = [
    0,
    16,
  ...
  if (cloud_cover_low === 0 && precipitation === 0){
    setNebulositeImg(nebulositeArray[0].name);
  ...
  },[cloud_cover_low, precipitation, setNebulositeText, setNebulositeImg]);
